function [spx_featureset] = feature_extraction(spx_leaf)
h1=size(spx_leaf,2);
spx_featureset_org=zeros(h1,624);

for i=1:h1
  spx=spx_leaf(i).superpixel;
  spx_featureset_org(i,:)= cooc3d(spx);
end

%----------------NAN value remove---------------
x=isnan(spx_featureset_org(1,:));
ex_n=624-nnz(x);
spx_featureset=zeros(h1,ex_n);

for i=1:h1
    c=1;
    for j=1:624
        if (isnan(spx_featureset_org(i,j))==0)
          spx_featureset(i,c)=spx_featureset_org(i,j);
          c=c+1;
        end
    end
end
